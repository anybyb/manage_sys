...
public class JedisClientSentinel  implements JedisClient{
	...
	private static final Logger logger = Logger.getLogger(JedisClientSentinel.class);
	...
	public String get(String key){
		...
		try {
			...
			logger.error("redis主从异常",e);
		} finally {
			returnResource(jedisSentinelPool, jedis);
		}
		return value;
	}
	 ...
	 * @param intValue
	 */
	public void expire(String key, int intValue) {
		Jedis jedis = null;
		try {
			jedis = jedisSentinelPool.getResource();
			jedis.expire(key,intValue);
		} catch (Exception e) {
			//pool.returnBrokenResource(jedis);
			logger.error("redis哨兵异常",e);
		} finally {
	...
	public static void returnResource(JedisSentinelPool pool, Jedis jedis) {
		if (jedis != null) {
			logger.debug("jedis返回JedisSentinlPool前正在被使用的jedis连接数:"+pool.getNumActive());
			jedis.close();
			logger.debug("jedis返回JedisSentinelPool后正在被使用的jedis连接数:"+pool.getNumActive());
		}
	}
