 ...
 * 手动与springboot集成使用
 * 单机模式redis 
 * @describe:  每次使用jedis后记得返回到jedisPool中
 * @author: wcc
 * @Time:2017年7月7日
 * @version 1.0
 * @company:
...
public class JedisClientSingle  implements JedisClient{
	...
	private static final Logger logger = Logger.getLogger(JedisClientSingle.class);
	...
	public static void returnResource(JedisPool pool, Jedis jedis) {
		if (jedis != null) {
			logger.debug("jedis返回JedisPool前正在被使用的jedis连接数:"+pool.getNumActive());
			jedis.close();
			logger.debug("jedis返回JedisPool后正在被使用的jedis连接数:"+pool.getNumActive());
		}
	}
